"""
S2-LageLive v3.0 · KI-gestütztes Lagedarstellungssystem
FwDV 102 / DV 100 / BBK 2024

Railway-kompatibel: PORT aus ENV, /health immer verfügbar
Alle optionalen Imports mit try/except abgesichert
"""
...
from fastapi import (
    ...
    Depends, HTTPException, UploadFile, File, Form, Request
)
...
# ── Konfiguration ───────────────────────────────────────────────
SECRET_KEY    = os.environ.get("SECRET_KEY", "s2-dev-key-bitte-in-railway-variables-setzen!")
ALGORITHM     = "HS256"
...
class TagebuchEintrag(Base):
    """Append-Only mit SHA-256 Hash-Chain — rechtssicher."""
...
class KartenZeichen(Base):
    ...
    tz_kategorie = Column(String(64), default="Sonstiges")
    ...
    with Session(engine) as s:
        if s.query(User).count() == 0:
            for u, pw, name, role in [
                ...
                s.add(User(username=u, hashed_pw=pwd_ctx.hash(pw), display_name=name, role=role))
            s.commit()
            print("✅ Nutzer angelegt (Passwörter vor Einsatz ändern!)")
        if s.query(Einsatz).count() == 0:
            s.add(Einsatz(kennung=f"E-{datetime.now():%Y%m%d}", stichwort="Bereitschaft", einsatzort="–"))
            s.commit()
...
def _eintrag_dict(r: TagebuchEintrag) -> dict:
    return {
        ...
        "vordruck_nr": r.vordruck_nr, "vordruck_von": r.vordruck_von, "vordruck_an": r.vordruck_an,
...
class Hub:
    ...
    async def connect(self, ws: WebSocket, eid: int, uid: str):
        ...
        try:
            await ws.send_json({
                "type": "connected", "eid": eid, "user": uid,
                "clients": len(self._c[eid]),
    ...
    async def broadcast(self, eid: int, msg: dict, exclude: WebSocket = None):
        ...
        for ws, uid in list(self._c.get(eid, [])):
            ...
            try:
                ...
                dead.append((ws, uid))
        if dead:
            self._c[eid] = [(w, u) for w, u in self._c.get(eid, []) if (w, u) not in dead]
...
_ORTE = {
    ...
    "hausach":    (48.2833, 8.1667, "🔴", "#C8000A"),
    "haslach":    (48.2667, 8.0833, "🔴", "#C8000A"),
}


def _geodaten_aus_text(text: str) -> list:
    result = []
    tl = text.lower()
    ...
    if m:
        try:
            ...
            if 47 < lat < 50 and 6 < lng < 10:
                result.insert(0, {"tz_typ": "Einsatzschwerpunkt", "farbe": "#C8000A",
...
async def ki_lage(eingaben: dict) -> dict:
    """KI-Analyse für Lageeingabe. Gibt strukturiertes Ergebnis zurück."""
    if not ANTHROPIC_KEY:
        return {
            "raw": "⚠ ANTHROPIC_API_KEY fehlt – in Railway Variables setzen.",
            "lagebeschreibung": (
                f"Lageeingabe empfangen. Stand: {datetime.now():%d.%m.%Y %H:%M Uhr}\n\n"
                f"Einsatzort: {eingaben.get('einsatzort','–')}\n"
                f"Stichwort: {eingaben.get('stichwort','–')}\n"
                f"Schadenslage: {eingaben.get('schadenslage','–')}"
            ),
            "geodaten": _geodaten_aus_text(
                eingaben.get("schadenslage","") + " " + eingaben.get("geoLage","")
            ),
        }
...
Erstelle alle gewünschten Produkte.
Trenne sie EXAKT mit === PRODUKTNAME === als Überschrift.
Stand {now} bei jedem Produkt vermerken.
Keine Markdown-Sternchen. DV 100 konform. Sachlich und belastbar.
Alle Ausgaben sind Vorschläge – Freigabe durch verantwortliches Personal erforderlich."""
    ...
    try:
        ...
        msg = client.messages.create(
            ...
            max_tokens=4000,
            system=(
                "Du bist KI-Unterstützung S2 gemäß DV 100 / FwDV 102 / BBK 2024. "
                "Alle Ausgaben sind Vorschläge. Keine Markdown-Sternchen. "
                "Trenne Produkte mit === ÜBERSCHRIFT ===."
            ),
    ...
    for sec in ["LAGEBESCHREIBUNG", "AKTUELLE LAGEBESCHREIBUNG", "KURZLAGE",
        ...
        if m:
            k = sec.lower().replace(" ", "_").replace("ü","ue").replace("ä","ae").replace("ö","oe")
            result[k] = m.group(1).strip()

    result["geodaten"] = _geodaten_aus_text(
        eingaben.get("schadenslage","") + " " +
        eingaben.get("geoLage","") + " " +
...
async def ki_dokument(text: str, dateiname: str, einsatz_kontext: dict) -> dict:
    """KI-Analyse für hochgeladene Dokumente."""
    if not ANTHROPIC_KEY:
        return {
    ...
    try:
        ...
        client = _ant.Anthropic(api_key=ANTHROPIC_KEY)
        now = f"{datetime.now():%d.%m.%Y %H:%M Uhr}"
        prompt = f"""Analysiere dieses Dokument für das Einsatztagebuch S2 (DV 100).
Dokument: {dateiname} | Stand: {now}

EINSATZKONTEXT: {json.dumps(einsatz_kontext, ensure_ascii=False)}

DOKUMENTINHALT:
{text[:3500]}

Antworte NUR mit validem JSON (kein Text davor/danach, kein Markdown):
{{
  "kategorie": "Lage|Maßnahme|Meldung|Vordruck|Information",
  "prioritaet": "kritisch|hoch|normal",
  "betreff": "Max 80 Zeichen",
  "inhalt": "Strukturierte Zusammenfassung",
  "ki_zusammenfassung": "2-3 Sätze Bedeutung für S2",
  "geodaten": []
}}"""
        msg = client.messages.create(
            model="claude-sonnet-4-20250514",
        ...
        return {
            ...
            "betreff": f"Dokument: {dateiname}",
            "inhalt": text[:500],
            "ki_zusammenfassung": f"Analyse: {e}",
            "geodaten": [],
        }


async def ki_einsatzplanung(einsatz_dict: dict, abschnitte: list, phasen: list) -> dict:
    """KI-gestützte Einsatzplanung."""
    if not ANTHROPIC_KEY:
        return {
            "ki_plan": "KI nicht verfügbar.",
            "einsatzbefehl": _einsatzbefehl_vorlage(einsatz_dict, abschnitte),
        }
    try:
        import anthropic as _ant
        client = _ant.Anthropic(api_key=ANTHROPIC_KEY)
...
EINSATZ:
{json.dumps(einsatz_dict, ensure_ascii=False, indent=2)}
...
Erstelle:
=== EINSATZPLAN ===
Vollständiger Einsatzplan nach DV 100: Lagebeurteilung, Führungsorganisation, Abschnittsaufgaben, kritische Punkte.

=== FÜHRUNGSRHYTHMUS ===
Besprechungsintervalle, Teilnehmer, Themen, Eskalationsstufen.

=== AUFBAU-FAHRPLAN ===
Priorisierte Aufbaumaßnahmen T+0 bis T+4h mit Verantwortlichkeiten.

=== TAKTISCHE HINWEISE ===
Besonderheiten, Koordinationsbedarfe, Dauerbetrieb.

Stand {now}. DV 100 konform. Alle Ausgaben sind Vorschläge."""
        msg = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=3000,
            system="Du bist S2-KI-Unterstützung gemäß DV 100. Alle Ausgaben sind Vorschläge.",
            messages=[{"role": "user", "content": prompt}],
        )
        text = msg.content[0].text
        return {
...
def _einsatzbefehl_vorlage(e: dict, abschnitte: list) -> str:
    now = f"{datetime.now():%d.%m.%Y %H:%M Uhr}"
...
S · SITUATION
  ...
  Lagestufe:  {e.get('lagestufe','–')}
  [Schadenslage aus Lagebeschreibung einfügen]

M · MISSION (AUFTRAG)
  Bewältigung der Lage {e.get('einsatzort','–')} unter Führung des Führungsstabs.

E · EXECUTION (DURCHFÜHRUNG)
{ab_text if ab_text else '  [Abschnitte noch nicht definiert]'}

A · ADMINISTRATION
  Führungsstelle: {e.get('fuehrstelle','[einzutragen]')}
  BSR Nord: [einzutragen] · BSR Süd: [einzutragen]
  Versorgung: [einzutragen]

K · KOMMANDO
  EL: {e.get('el_name','[einzutragen]')}
  Nächste Lagebesprechung: [einzutragen]

ENTWURF · Freigabe durch EL erforderlich · Stand {now}"""


def _std_abschnitte(stichwort: str) -> list:
    stw = (stichwort or "").upper()
    if "MANV" in stw or "KAT" in stw:
        return [
            {"nummer":"A1","bezeichnung":"Patientenversorgung/BHP","aufgabe":"Sichtung, Behandlung, Transport","status":"aktiv"},
            {"nummer":"A2","bezeichnung":"Rettung/Bergung","aufgabe":"Verschüttete befreien","status":"aktiv"},
            {"nummer":"A3","bezeichnung":"Absperrung/Ordnung","aufgabe":"Zufahrten sichern","status":"aktiv"},
            {"nummer":"A4","bezeichnung":"Betreuung/Unterkunft","aufgabe":"Obdachlose versorgen","status":"aktiv"},
            {"nummer":"LOG","bezeichnung":"Logistik/Versorgung","aufgabe":"Nachschub, Ablösung","status":"aktiv"},
        ]
    if "HW" in stw or "WASSER" in stw or "DAMM" in stw or "ÜSG" in stw:
        return [
...
def _std_phasen(stichwort: str) -> list:
    stw = (stichwort or "").upper()
    basis = [
        {"nr":1,"name":"Erstmaßnahmen","dauer":"0–30 min","aufgaben":["Lageüberblick","Führungsstelle einrichten","Erkundung","Erstmeldung"]},
        {"nr":2,"name":"Führungsaufbau","dauer":"30–90 min","aufgaben":["Sachgebiete besetzen","Abschnitte einteilen","Kommunikationskonzept","Lagebesprechung 1"]},
        {"nr":3,"name":"Einsatzbetrieb","dauer":"laufend","aufgaben":["Führungsrhythmus","Lagefortschreibung","Ressourcenmanagement","Medienarbeit"]},
        {"nr":4,"name":"Rückzug/Nachbereitung","dauer":"variabel","aufgaben":["Kräfte abziehen","Übergabe","Abschlussbericht","Tagebuch schließen"]},
    ]
    if "MANV" in stw or "KAT" in stw:
        basis[0]["aufgaben"].insert(0, "MANV-Leitung einsetzen")
        basis[0]["aufgaben"].insert(1, "BHP einrichten")
    if "HW" in stw or "DAMM" in stw:
        basis[0]["aufgaben"].insert(0, "Pegelstand überwachen")
        basis[0]["aufgaben"].insert(1, "Evakuierungszonen festlegen")
    return basis


# ════════════════════════════════════════════════════════════════
# TEXT-EXTRAKTION aus Uploads
# ════════════════════════════════════════════════════════════════
...
def _text_aus_bytes(content: bytes, filename: str) -> str:
    ...
    if suffix == ".pdf":
        try:
            ...
            return f"[PDF: {filename} – PyPDF2 nicht installiert. pip install pypdf2]"
        except Exception as e:
...
def _erkenne_vordruck(text: str) -> str:
    tl = text.lower()
    muster = {
        "Lagemeldung":     ["lagemeldung","lagenstatus"],
        "Einsatzbefehl":   ["einsatzbefehl","gesamtbefehl"],
        "Kräfteübersicht": ["kräfteübersicht","kräfte und mittel"],
        "Vierfachvordruck":["vierfachvordruck","von:","an:","betreff:"],
        "Presseinfo":      ["pressemitteilung","presseinfo"],
        "Abschlussbericht":["abschlussbericht","einsatzende"],
    }
    for typ, keys in muster.items():
...
def req(*roles: str):
    def dep(user: User = Depends(get_user)) -> User:
...
async def lifespan(app: FastAPI):
    init_db()
    print(f"🚒 S2-LageLive v3.0 · Port {PORT} · {datetime.now():%d.%m.%Y %H:%M}")
    yield

app = FastAPI(
    title="S2-LageLive",
    version="3.0.0",
    description="KI-Lagedarstellungssystem FwDV 102 / DV 100",
    lifespan=lifespan,
)
...
async def login(form: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    return {
        "access_token": _create_token({"sub": u.username, "role": u.role}),
        "token_type": "bearer",
...
async def list_e(db: Session = Depends(get_db), u: User = Depends(get_user)):
    ...
    return [{"id":e.id,"kennung":e.kennung,"stichwort":e.stichwort,"einsatzort":e.einsatzort,
             ...
             "alarmzeit":e.alarmzeit.isoformat() if e.alarmzeit else None} for e in rows]
...
async def list_tb(eid: int, db: Session = Depends(get_db), u: User = Depends(get_user)):
    ...
    return [_eintrag_dict(r) for r in rows]
...
async def add_tb(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(get_user)):
    r = TagebuchEintrag(
        ...
        vordruck_an=data.get("vordruck_an"), prev_hash=_prev_hash(db,eid),
    )
    db.add(r); db.flush(); r.entry_hash = r.berechne_hash(); db.commit(); db.refresh(r)
    msg = {**_eintrag_dict(r), "type":"tagebuch_eintrag",
...
async def add_karte(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(get_user)):
    k = KartenZeichen(einsatz_id=eid, tz_typ=data.get("tz_typ","Marker"),
                      tz_kategorie=data.get("tz_kategorie","Sonstiges"),
...
async def ki_route(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(req("s2"))):
    ...
    # Tagebucheintrag
    inhalt = (result.get("lagebeschreibung") or result.get("aktuelle_lagebeschreibung") or result.get("raw","–"))
    r = TagebuchEintrag(einsatz_id=eid, author_name="KI-System S2", author_role="system",
                        ...
                        inhalt=inhalt[:2000], quelle="KI-Analyse", prev_hash=_prev_hash(db,eid))
    db.add(r); db.flush(); r.entry_hash=r.berechne_hash()
    # Geodaten auf Karte
    for geo in result.get("geodaten",[]):
        if not(geo.get("lat") and geo.get("lng")): continue
        kz = KartenZeichen(einsatz_id=eid, tz_typ=geo.get("tz_typ","Marker"),
                           farbe=geo.get("farbe","#C8000A"), emoji=geo.get("emoji","📍"),
                           lat=geo["lat"], lng=geo["lng"],
                           beschreibung=geo.get("beschreibung",""), erstellt_von="KI-Agent")
        db.add(kz); db.flush()
        await hub.broadcast(eid,{"type":"karten_zeichen","id":kz.id,**geo,
                                  "zeit":datetime.now().strftime("%H:%M")})
    db.commit()
    await hub.broadcast(eid,{"type":"einsatz_update","einsatz_id":eid,
                              "tote":e.tote if e else 0,"verletzte":e.verletzte if e else 0,
                              "verschuettete":e.verschuettete if e else 0,"obdachlose":e.obdachlose if e else 0})
    await hub.broadcast(eid,{"type":"ki_analyse_fertig",**result})
    return result

# ── DATEI-UPLOAD & AUTOMATISCHER INGEST ─────────────────────────
...
async def upload(eid: int, file: UploadFile = File(...),
    ...
    vordruck = _erkenne_vordruck(text)
    await hub.broadcast(eid,{"type":"datei_eingang","datei":file.filename,
                              "vordruck_typ":vordruck,"ki_kurz":"Analyse läuft …",
                              "zeit":datetime.now().strftime("%H:%M")})
    asyncio.create_task(_ingest_task(eid, file.filename or safe, text, vordruck, u.display_name or u.username))
    return {"datei":file.filename,"vordruck_typ":vordruck,"status":"wird verarbeitet"}

async def _ingest_task(eid: int, filename: str, text: str, vordruck: str, author: str):
    try:
        ...
        with Session(engine) as db:
            r = TagebuchEintrag(
                einsatz_id=eid, author_name=f"Ingest: {filename}", author_role="system",
                ...
                quelle=f"Upload: {filename}", vordruck_nr=vordruck,
...
async def planung_ki(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(req("s1","s2"))):
    e = db.get(Einsatz, eid)
    if not e: raise HTTPException(404)
    ...
    result = await ki_einsatzplanung(edict, abschnitte, phasen)
    # Speichern
    p = db.query(Einsatzplanung).filter(Einsatzplanung.einsatz_id==eid).first()
    if not p:
        p = Einsatzplanung(einsatz_id=eid)
        db.add(p)
    ...
    p.geaendert_dt=datetime.utcnow()
    db.commit()
    # Tagebucheintrag
    with Session(engine) as db2:
        r = TagebuchEintrag(einsatz_id=eid, author_name="KI-Einsatzplanung", author_role="system",
                            kategorie="Maßnahme", prioritaet="hoch",
                            betreff=f"Einsatzplanung erstellt · {datetime.now():%H:%M Uhr}",
    ...
    await hub.broadcast(eid,{"type":"einsatzplanung_fertig","ki_plan":result.get("ki_plan",""),
                              "einsatzbefehl":result.get("einsatzbefehl",""),"phasen":phasen,"abschnitte":abschnitte})
    return result

@app.put("/api/einsaetze/{eid}/planung")
async def update_planung(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(req("s1","s2"))):
    p = db.query(Einsatzplanung).filter(Einsatzplanung.einsatz_id==eid).first()
    if not p: p = Einsatzplanung(einsatz_id=eid); db.add(p)
    if "phasen"    in data: p.phasen_json=json.dumps(data["phasen"],ensure_ascii=False)
    if "abschnitte"in data: p.abschn_json=json.dumps(data["abschnitte"],ensure_ascii=False)
    p.geaendert_dt=datetime.utcnow(); db.commit()
    return {"ok":True}

# ── AUSGABE-PIPELINE ─────────────────────────────────────────────
AUSGABE_TYPEN = {
    "lagebericht":       {"name":"Lagebericht (DV 100)",          "icon":"📋"},
    "lagevortrag":       {"name":"Lagevortrag",                   "icon":"🎤"},
    "kurzlage":          {"name":"Kurzlage",                      "icon":"⚡"},
    "management_summary":{"name":"Management Summary",            "icon":"👔"},
    "pressemitteilung":  {"name":"Pressemitteilung",              "icon":"📰"},
    "buergerinfo":       {"name":"Bürgerinformation",             "icon":"🏘"},
    "verhaltensempfehlung":{"name":"Verhaltensempfehlung",        "icon":"⚠️"},
    "fuehrungs_unterlage":{"name":"Führungsunterlage/TOPs",       "icon":"📅"},
    "einsatzbericht":    {"name":"Einsatzbericht (Abschluss)",    "icon":"🗂"},
    "einsatzbefehl":     {"name":"Einsatzbefehl (Entwurf)",       "icon":"📜"},
}

AUSGABE_PROMPTS = {
    "lagebericht":        "Vollständiger Lagebericht nach DV 100: 1.Eigene Lage 2.Schadenslage 3.Gefahrenlage 4.Kräfteansatz 5.Geographische Lage 6.Maßnahmen 7.Lageentwicklung 8.Informationsbedarfe",
    "lagevortrag":        "Sprechbares Manuskript für Lagebesprechung (5-10 Min): 1.Aktuelle Lage 2.Kräfteansatz 3.Maßnahmen 4.Offene Punkte 5.Nächste Schritte. Für nicht-technisches Publikum.",
    "kurzlage":           "Kurzlage max. 1 Seite, Bulletpoints: Was·Wo·Wie viele·Welche Kräfte·Was unklar·Nächster Schritt.",
    "management_summary": "Management Summary für Bürgermeister/Landrat: Kein Fachjargon, klare Zahlen, Was wird getan, Was gebraucht wird, Politischer Handlungsbedarf. Max 1 Seite.",
    "pressemitteilung":   "Pressemitteilung ENTWURF: Titel, Datum/Ort, Lead, Hintergrund, Maßnahmen, Zitat-Platzhalter, Verhaltenshinweise, Kontakt. NUR belegte Fakten. Als ENTWURF kennzeichnen.",
    "buergerinfo":        "Bürgerinformation einfache Sprache: Was ist passiert, Was TUN (Liste), Was NICHT TUN, Wo gibt es Info, Notrufnummern. Max 15 kurze Sätze.",
    "verhaltensempfehlung":"Verhaltensempfehlungen nummerierte Liste: Sofortmaßnahmen, Selbstschutz, Was mitnehmen, Kommunikation. Beruhigend aber klar.",
    "fuehrungs_unterlage":"Führungsunterlage für nächste Lagebesprechung: Tagesordnung, Lageinformation je SG, Offene Fragen, Entscheidungsbedarfe.",
    "einsatzbericht":     "Einsatzabschlussbericht: Einsatzgrundlagen, chronologischer Ablauf, Kräfte, Maßnahmen, Ergebnisse, Bewertung, Empfehlungen.",
    "einsatzbefehl":      "Einsatzbefehl nach SMEAK: S=Situation, M=Mission, E=Execution, A=Administration, K=Kommando. Als ENTWURF kennzeichnen.",
}

@app.get("/api/ausgaben/typen")
async def get_ausgabe_typen(u: User = Depends(get_user)):
    return [{"id":k,"name":v["name"],"icon":v["icon"]} for k,v in AUSGABE_TYPEN.items()]

@app.post("/api/einsaetze/{eid}/ausgaben")
async def erstelle_ausgaben(eid: int, data: dict, db: Session = Depends(get_db), u: User = Depends(req("s2"))):
    ...
    lage = {
        ...
        **{k:data.get(k,"–") for k in ["schadenslage","gefahrenlage","kraefte","geo","wetter","massnahmen"]},
    }
    now = f"{datetime.now():%d.%m.%Y %H:%M Uhr}"
    ki_text = "[KI nicht verfügbar]"
    if ANTHROPIC_KEY:
        try:
            import anthropic as _ant
            ausgaben_block = "\n\n".join(
                f"=== {AUSGABE_TYPEN.get(a,{}).get('name',a).upper()} ===\n"
                f"Adressat: {a}\n{AUSGABE_PROMPTS.get(a,'Erstelle das Produkt.')}"
                for a in gewaehlte
            )
            prompt = f"""LAGESTAND {now}:
{json.dumps(lage,ensure_ascii=False,indent=2)}

ERSTELLE FOLGENDE PRODUKTE — JEDE MIT === NAME === ÜBERSCHRIFT UND STAND {now}:
{ausgaben_block}

Alle Ausgaben sachlich, DV 100 konform. Als ENTWURF kennzeichnen wo zutreffend."""
            client = _ant.Anthropic(api_key=ANTHROPIC_KEY)
            msg = client.messages.create(model="claude-sonnet-4-20250514", max_tokens=5000,
                system="Du bist S2-KI-Unterstützung DV 100. Alle Ausgaben sind Entwürfe.",
                messages=[{"role":"user","content":prompt}])
            ki_text = msg.content[0].text
        except Exception as ex:
            ki_text = f"KI-Fehler: {ex}"
    # Speichern
    path_txt = f"exports/ausgaben_{eid}_{datetime.now():%Y%m%d_%H%M}.txt"
    Path(path_txt).write_text(ki_text, encoding="utf-8")
    # Tagebucheintrag
    namen = [AUSGABE_TYPEN.get(a,{}).get("name",a) for a in gewaehlte[:3]]
    with Session(engine) as db2:
        r = TagebuchEintrag(einsatz_id=eid, author_name="Ausgabe-Pipeline", author_role="system",
                            kategorie="Lage", prioritaet="normal",
                            betreff=f"KI-Ausgaben: {', '.join(namen)} · {datetime.now():%H:%M}",
                            inhalt=ki_text[:1500], quelle="Ausgabe-Pipeline",
                            prev_hash=_prev_hash(db2,eid))
        db2.add(r); db2.flush(); r.entry_hash=r.berechne_hash(); db2.commit()
    await hub.broadcast(eid,{"type":"ausgaben_fertig","ausgaben":gewaehlte,"ki_text":ki_text[:500]+"…"})
    return {"ki_text":ki_text,"ausgaben":gewaehlte,"export":path_txt}

@app.get("/api/einsaetze/{eid}/ausgaben/download")
async def download_ausgaben(eid: int, db: Session = Depends(get_db), u: User = Depends(get_user)):
    import glob
    ...
    return FileResponse(files[0], media_type="text/plain", filename=f"Ausgaben-E{eid}.txt")
...
async def root():
    ...
    return JSONResponse({"status":"S2-LageLive läuft","health":"/health","docs":"/docs"})
